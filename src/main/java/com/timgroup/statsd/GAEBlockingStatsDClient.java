...
public final class GAEBlockingStatsDClient implements StatsDClient {
    ...
    private static final ThreadLocal<NumberFormat> NUMBER_FORMATTERS = new ThreadLocal<NumberFormat>() {
        ...
        protected NumberFormat initialValue() {
            ...
            // we need to specify a value for Double.NaN that is recognized by
            // dogStatsD
            if (numberFormatter instanceof DecimalFormat) { // better safe than
                                                            // a runtime error
     ...
     * exceptions thrown during subsequent usage are consumed, guaranteeing that
     * failures in metrics will not affect normal code execution.
     *
     * @param prefix
     ...
     */
    public GAEBlockingStatsDClient(final String prefix, final String hostname, final int port)
            throws StatsDClientException {
     ...
     *            the port of the targeted StatsD server
     * @param queueSize
     *            the maximum amount of unprocessed messages in the
     *            BlockingQueue.
     * @throws StatsDClientException
     *             if the client could not be started
     */
     ...
     *            tags to be added to all content sent
     * @param errorHandler
     *            handler to use when an exception occurs during usage, may be
     *            null to indicate noop
     * @throws StatsDClientException
     *             if the client could not be started
     */
    public GAEBlockingStatsDClient(final String prefix, final String hostname, final int port,
            ...
            throws StatsDClientException {
        this(prefix, Integer.MAX_VALUE, constantTags, errorHandler, staticStatsDAddressResolution(
                hostname, port));
    }
     ...
     */
    public GAEBlockingStatsDClient(final String prefix, final String hostname, final int port,
            final int queueSize, final String[] constantTags,
            final StatsDClientErrorHandler errorHandler) throws StatsDClientException {
        this(prefix, queueSize, constantTags, errorHandler, staticStatsDAddressResolution(hostname,
                port));
    }
     ...
     *             if the client could not be started
     */
    public GAEBlockingStatsDClient(final String prefix, final int queueSize, String[] constantTags,
        ...
        try {
            hostAddress = addressLookup.call();
     ...
     * {@link #incrementCounter(String, String[])}.
     */
    @Override
    ...
    /**
     * Convenience method equivalent to
     * {@link #decrementCounter(String, String[])}.
     */
    @Override
    ...
    public void recordGaugeValue(final String aspect, final double value, final String... tags) {
        /*
         * Intentionally using %s rather than %f here to avoid padding with
         * extra 0s to represent precision
         */
        send(String.format("%s%s:%s|g%s", prefix, aspect, NUMBER_FORMATTERS.get().format(value),
                tagString(tags)));
    }
    ...
    public void recordGaugeValue(final String aspect, final double value, final double sampleRate,
        ...
        send(String.format("%s%s:%s|g|@%f%s", prefix, aspect,
                NUMBER_FORMATTERS.get().format(value), sampleRate, tagString(tags)));
    }

    /**
     * Convenience method equivalent to
     * {@link #recordGaugeValue(String, double, String[])}.
     */
    @Override
    ...
    /**
     * Convenience method equivalent to
     * {@link #recordGaugeValue(String, long, String[])}.
     */
    @Override
    ...
    public void recordExecutionTime(final String aspect, final long timeInMs,
            final double sampleRate, final String... tags) {
        if (isInvalidSample(sampleRate)) {
            return;
        }
        send(String.format("%s%s:%d|ms|@%f%s", prefix, aspect, timeInMs, sampleRate,
                tagString(tags)));
    }

    /**
     * Convenience method equivalent to
     * {@link #recordExecutionTime(String, long, String[])}.
     */
    @Override
    ...
    public void recordHistogramValue(final String aspect, final double value, final String... tags) {
         ...
         * extra 0s to represent precision
         */
        send(String.format("%s%s:%s|h%s", prefix, aspect, NUMBER_FORMATTERS.get().format(value),
                tagString(tags)));
    }
    ...
    public void recordHistogramValue(final String aspect, final double value,
         ...
         * extra 0s to represent precision
         */
        send(String.format("%s%s:%s|h|@%f%s", prefix, aspect,
                NUMBER_FORMATTERS.get().format(value), sampleRate, tagString(tags)));
    }

    /**
     * Convenience method equivalent to
     * {@link #recordHistogramValue(String, double, String[])}.
     */
    @Override
     ...
     */
    @Override
    public void recordHistogramValue(final String aspect, final long value,
            final double sampleRate, final String... tags) {
        if (isInvalidSample(sampleRate)) {
            return;
        }
    ...
    /**
     * Convenience method equivalent to
     * {@link #recordHistogramValue(String, long, String[])}.
     */
    @Override
     ...
     *
     * @see <a
     *      href="http://docs.datadoghq.com/guides/dogstatsd/#events-1">http://docs.datadoghq.com/guides/dogstatsd/#events-1</a>
     */
    @Override
    public void recordEvent(final Event event, final String... tags) {
        ...
        send(String.format("_e{%d,%d}:%s|%s%s%s", title.length(), text.length(), title, text,
                eventMap(event), tagString(tags)));
    }
    ...
    /**
     * Convenience method equivalent to
     * {@link #recordServiceCheckRun(ServiceCheck sc)}.
     */
    @Override
     ...
     *
     * Sets are used to count the number of unique elements in a group. If you
     * want to track the number of unique visitor to your site, sets are a great
     * way to do that.
     *
     * <p>
     * This method is a DataDog extension, and may not work with other servers.
    ...
    public void recordSetValue(final String aspect, final String value, final String... tags) {
        // documentation is light, but looking at dogstatsd source, we can send
        // string values
    ...
    private void sendTrackMessage(byte[] data) throws IOException {
     ...
     * @throws Exception
     *             if the lookup fails, i.e. {@link UnknownHostException}
     */
    private static Callable<InetSocketAddress> staticAddressResolution(final String hostname,
    ...
    private static Callable<InetSocketAddress> staticStatsDAddressResolution(final String hostname,
